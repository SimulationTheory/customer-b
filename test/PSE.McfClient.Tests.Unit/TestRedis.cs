...
namespace PSE.McfClient.Tests.Unit
{
    ...
    {
        class TestRedis : IDatabase
        {
            Dictionary<RedisKey, RedisValue> _cache = new Dictionary<RedisKey, RedisValue>();
            public int Database => -1;
            public RedisValue StringGet(RedisKey key, CommandFlags flags = CommandFlags.None)
            {
                if (_cache.TryGetValue(key, out var cookie))
                {
                    return (RedisValue)cookie;
                }
